...
import { Sun, Moon, BookOpen, Highlighter, FileText } from "lucide-react";
...
export function Profile() {
  ...
  const { darkMode, toggleDarkMode, highlights, notes, readingProgress } = useBible();

  // Filter notes ที่มี verse จริง
  const validNotes = notes
    .map(n => ({ ...n, verse: getVerseById(n.verseId) }))
    .filter(n => n.verse !== null);

  const stats = [
    { label: 'Highlights', value: highlights.size, icon: Highlighter, color: 'text-yellow-600 dark:text-yellow-500' },
    { label: 'Notes', value: validNotes.length, icon: FileText, color: 'text-blue-600 dark:text-blue-400' },
  ];
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {stats.map(stat => {
              ...
              return (
                <div
                  key={stat.label}
                  className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-5 border border-gray-100 dark:border-gray-700 cursor-pointer hover:scale-105 transition-transform"
                  onClick={() => stat.label === "Highlights" ? navigate("/highlights") : navigate("/notes")}
                >
